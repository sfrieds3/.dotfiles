...
alias knt 'kubectl get nodes -o custom-columns=NAME:.metadata.name,TAINTS:.spec.taints --no-headers'
...
alias klo 'kubectl logs --follow'
alias klop 'kubectl logs --follow --previous'
...
# alias tk "for s in (tmux list-sessions | awk '{print \$1}' | sed s/:\$// | fzf); tmux kill-session -t \$s; end"
...
alias set-pudb-breakpoint 'set -x PYTHONBREAKPOINT "pudb.set_trace"'
alias unset-pudb-breakpoint 'set -e PYTHONBREAKPOINT'
